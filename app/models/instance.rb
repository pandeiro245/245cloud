class Instance < ApplicationRecord
  ...
  def fetch_comments
    ...
    data.each do |record|
      process_record(record, Comment, find_keys: %w[created_at user_id]) do |comment, data|
        data = data.except('id')
  ...
  def fetch_comments_all
    fetch_paginated_data('api/comments/download.json', Comment, find_keys: %w[created_at user_id]) do |comment, data|
      data.each { |key, val| comment.send("#{key}=", val) }
      comment.save!(validate: false)
    end
  end
