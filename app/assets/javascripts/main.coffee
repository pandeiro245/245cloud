...
initHeatmap = () ->
  ...
  cal.init({
    ...
    afterLoad: () ->
      pomos = {}
      $.get("/api/users/#{window.facebook_id}/workloads?type=done&limit=99999", (workloads) ->
        pomos = {}
  ...
  $.get('/timecrowd/recents', (data) ->
    ...
    if data.status == 'ng'
      ...
      for entry in data.entries
        ...
        continue if !entry.task.url.match(/github.com/) and !entry.task.url.match(/trello.com/) and !entry.task.url.match(/cybozu.com/) and !entry.task.url.match(/ruffnote.com/)
